...
This is a [`block`](https://witheve.github.io/docs/handbook/block) of Eve code. Blocks compose Eve programs, and are delineated by code fences, which are three backticks (```) in a row on a newline before and after the code (click [here](https://github.com/witheve/docs/blob/draft/src/guides/quickstart.md) to see the source of this file to see the code fences). Everything inside a fence is code, everything outside of a fence is treated as a comment.  
